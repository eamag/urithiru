...
<aside class="sidebar">
  <a class="brand" href="/" aria-label="About Urithiru">
    <span class="brand-mark"></span>
    <span><b>URITHIRU</b><small>exploration engine</small></span>
  </a>

  <nav aria-label="Workspace">
    <button class:active={view === "runs"} type="button" onclick={onruns}>All runs <span>{entries.length}</span></button>
  </nav>

  <label class="search">
    <span class="sr-only">Search runs</span>
    <input bind:value={query} placeholder="Search runs" />
  </label>
  ...
  <div class="run-groups">
    ...
    {:else}
      <p class="none">No published runs yet.</p>
    {/each}
  </div>
...
<style>
  ...
  .brand { height: 72px; display: flex; align-items: center; gap: 11px; padding: 0 20px; color: inherit; background: transparent; text-align: left; text-decoration: none; }
