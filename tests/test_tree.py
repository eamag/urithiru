"""Tests for MCTS tree structure, UCT calculation, progressive widening, and backpropagation."""
...
def test_node_progressive_widening():
    ...
    # Empty queue and tried -> cannot widen
    assert not node.can_widen()

    node.queue = ["cand_1"]
    # No children yet -> can widen
    ...
    # 1 child, visits = 0 -> ceil(1.5 * 0^0.5) = 0 -> len(children) (1) is not < 0 -> cannot widen
    node.visits = 0
    assert not node.can_widen()

    # visits = 1 -> ceil(1.5 * 1) = 2 -> len(children) (1) < 2 -> can widen
    node.visits = 1
    assert node.can_widen()

    # visits = 4 -> ceil(1.5 * 2) = 3 -> can hold up to 2 children before needing more visits
...
def test_mcts_tree_select_and_backpropagate():
    ...
    # Root has no children and cannot widen initially
    assert tree.select() == tree.root

    # Add candidates to root queue
    tree.root.queue = ["c_1", "c_2"]
    # Now select returns root because root can widen
    assert tree.select() == tree.root
    ...
    # If root cannot widen (e.g. root visits=0, already has 2 children), select chooses child with highest UCT
    # Since child1 has visits=0, it has inf UCT
...
def test_mcts_tree_select_skips_terminal():
    ...
    c1.terminal = True
    c1.visits = 1
    c1.value = 10.0  # very high value, but terminal
