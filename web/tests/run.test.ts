...
describe("evaluation verdicts and stage chains", () => {
  ...
  it("assigns correct verdict strings", () => {
    ...
    expect(verdict(evNoExt)).toBe("no external result");

    const evAligned: Evaluation = {
      ...evNoExt,
      external: {
        source: "openalex",
        category_counts: makeCounts(0, 0, 0, 0, 30),
    ...
    expect(verdict(evAligned)).toBe("aligned");

    const evOpposed: Evaluation = {
      ...evNoExt,
      external: {
        source: "openalex",
    ...
    expect(verdict(evOpposed)).toBe("opposed");
  });
...
describe("the finding the landing page leads with", () => {
  function found(experiment: number, external: number) {
    return { node: "node_000002", claim: "a claim", prior: 0.7, literature: 0.8, experiment, external };
  }

  function entry(id: string, highlight: ReturnType<typeof found> | null): IndexEntry {
    return { id, status: "completed", updated_at: "", dataset: [], highlight };
  }

  it("picks the run whose external check moved the belief furthest", () => {
    const lead = leadFinding([
      entry("a", found(0.9, 0.8)),
      entry("b", found(0.98, 0.06)),
      entry("c", found(0.4, 0.6)),
    ]);
    expect(lead?.entry.id).toBe("b");
  });

  it("ignores runs that have no highlight yet", () => {
    const lead = leadFinding([entry("a", null), entry("b", found(0.9, 0.2))]);
    expect(lead?.entry.id).toBe("b");
    expect(leadFinding([entry("a", null)])).toBeNull();
    expect(leadFinding([])).toBeNull();
  });

  it("calls the external result opposed only across the midpoint", () => {
    expect(opposed(found(0.98, 0.06))).toBe(true);
    expect(opposed(found(0.98, 0.62))).toBe(false);
    expect(opposed(found(0.27, 0.17))).toBe(false);
  });

  it("measures the move as a distance, whichever way it went", () => {
    expect(moved(found(0.9, 0.2))).toBeCloseTo(0.7);
    expect(moved(found(0.2, 0.9))).toBeCloseTo(0.7);
  });
});

describe("what a tree node says it is", () => {
  const counts = (d_false = 0, m_false = 0, uncert = 0, m_true = 0, d_true = 0): Counts => ({
    "definitely false": d_false,
    "maybe false": m_false,
    uncertain: uncert,
    "maybe true": m_true,
  ...
  const experiment = (support: boolean, votes: Counts) => ({
    execution_success: true,
    test_spec_valid: true,
    direction_supported: support,
    empirical_support: support,
    category_counts: votes,
    rationale: "",
    metrics: {},
    p_value: 0.01,
    p_value_corrected: 0.01,
  ...
  const evaluation = (support: boolean, votes: Counts, external: Counts | null): Evaluation => ({
    prior: { counts: counts(0, 0, 30, 0, 0), rationale: "" },
    literature: { category_counts: counts(0, 0, 30, 0, 0), rationale: "", findings: {} },
    experiment: experiment(support, votes),
    external: external === null ? null : { source: "", category_counts: external, rationale: "", estimated_cost: 0, summary: "" },
    external_error: null,
    surprisal: { kl_search_param: 0, kl_code_search: 0.36, kl_code_param: 0, r_ice_norm: 0, belief_change: 0, is_surprising: true },
    reward: 0,
    external_value: 0,
  });
  const node = (evalu: Evaluation | null): CheckpointNode => ({
    id: "node_000008",
    parent_id: "node_000000",
    claim: "",
    visits: 1,
    value: 0,
    terminal: false,
  ...
  it("labels a node when the external result points the other way", () => {
    const overturned = node(evaluation(true, counts(0, 0, 0, 0, 30), counts(30, 0, 0, 0, 0)));
    expect(verdict(overturned.evaluation!)).toBe("opposed");
    expect(nodeState(overturned, none)).toBe("external-opposed");
  });

  it("labels an external result that opposes an agent-reported refutation", () => {
    const flipped = node(evaluation(false, counts(30, 0, 0, 0, 0), counts(0, 0, 0, 0, 30)));
    expect(nodeState(flipped, none)).toBe("external-opposed");
  });

  it("keeps aligned nodes on their agent-reported empirical label", () => {
    const agreed = node(evaluation(true, counts(0, 0, 0, 0, 30), counts(0, 0, 0, 0, 30)));
    expect(nodeState(agreed, none)).toBe("agent-supported");
    const bothFalse = node(evaluation(false, counts(30, 0, 0, 0, 0), counts(30, 0, 0, 0, 0)));
    expect(nodeState(bothFalse, none)).toBe("agent-refuted");
  });

  it("leaves a node with no external result on its agent-reported label", () => {
    expect(nodeState(node(evaluation(true, counts(0, 0, 0, 0, 30), null)), none)).toBe("agent-supported");
  });

  it("still reports the lifecycle states for nodes with no evaluation", () => {
    const waiting = node(null);
    expect(nodeState(waiting, new Set(["node_000008"]))).toBe("running");
    expect(nodeState(waiting, none)).toBe("queued");
    expect(nodeState({ ...waiting, terminal: true }, none)).toBe("exhausted");
    expect(nodeState({ ...waiting, parent_id: null }, none)).toBe("root");
  });
});
