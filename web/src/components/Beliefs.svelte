<script lang="ts">
  ...
  // The line stretches to the container, so only it lives in the SVG. Round dots drawn
  // in a stretched viewBox come out as ellipses, so those are positioned in CSS instead.
  const INSET = 4;
...
<!-- The whole argument in one shape: what was believed with no evidence, from the
     literature alone, after the experiment ran, and against independent data. -->
