"""Cloud Storage transfers and Cloud Run job control.

`CloudSandbox` in `runtime.sandbox` is the only caller.
"""
...
# The originals already live at the run prefix; re-uploading them per goal is waste.
...
class GoogleCloud:
    ...
    def generation_of(self, blob) -> int:
        """Zero is Cloud Storage's precondition for 'this object must not exist yet'."""
        try:
    ...
    def load_checkpoint(self, directory: Path) -> None:
        for name in CHECKPOINT_FILES:
            try:
                ...
                # A new run has no checkpoint or model caches yet.
    ...
    def submit(self, job: str, arguments: list[str], seconds: int):
        ...
        # Never retry a non-idempotent job launch after an ambiguous transport failure.
