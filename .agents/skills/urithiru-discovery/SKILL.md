...
3. **Data-Blind Literature Search ($P_{\text{search}}$) & Empirical Code Analysis ($P_{\text{code}}$)**: Started concurrently with separate workspaces. Local Docker stages share the operator login volume; Cloud stages receive separate `HOME` directories.
4. **Surprisal & External Challenge ($P_{\text{external}}$)**: If the literature and empirical findings diverge significantly, an external agent searches repositories (Zenodo, OpenAlex, Dryad, etc.) for a compatible dataset collected elsewhere.

The agents report metrics, validity flags and source provenance. Urithiru retains their
code and logs, but the host does not certify those claims; verify them before presenting
a finding as confirmed, replicated or based on an independent cohort.
...
Ensure Docker is running and `VERTEX_API_KEY` is exported. It covers the host's
prior, embedding and deduplication calls:
...
Copy the profile once and fill in your own identifiers (`configs/google.toml` ships
with placeholders and is refused until they are replaced; `*.local.toml` is
gitignored). See `docs/google.md` for the bucket, service account and job setup:

```bash
cp configs/google.toml configs/google.local.toml
# edit configs/google.local.toml: set project, region, bucket, orchestrator_job
...
_Note: This prints a reference `gs://bucket/prefix/<run-id>`._
...
Events emitted (`stage_started`, `agent_started`, `agent_finished`,
`stage_completed`, `stage_failed`, `stage_backoff`, `proposals_ready`,
`deduplicated`, `hypothesis_selected`, `external_skipped`, `hypothesis_evaluated`,
`run_finished`):

`hypothesis_evaluated` carries the evaluation summary and surprisal diagnostics —
the 4-hop beliefs (`p_param`, `p_search`, `p_code`, `p_external`), reward and verdict.
...
1. **An Agent-Reported Refutation is a Useful Lead, Not a Runtime Failure**:
   - If `empirical_support = false` and `direction_supported = false`, the agent reports that its analysis refuted the proposed hypothesis. Inspect the generated code and metrics before accepting that conclusion.
...
## 4. Iterative Deepening (Extending a Run)
...
- _Rule: The budget can only grow (monotonically non-decreasing invariant)._

---

## 5. Synthesis & Report Export
...
4. **Literature & External Context**: Key citations and any agent-reported external evidence, with provenance reviewed by the operator.
