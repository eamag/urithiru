"""Tests for checkpoint round-tripping, tree restoration, and budget growth guards."""
...
def test_engine_checkpoint_save_and_load(tmp_path):
    ...
    # Populate tree with 1 completed node
    ...
    # Create new engine over same directory to test restore
...
def test_engine_budget_growth_guard(tmp_path):
    ...
    # Attempting to load with a lower budget (3 < 5) must raise ValueError
    ...
    # Attempting to load with a grown budget (e.g. 6 >= 5) must succeed
...
def test_engine_seed_and_visit_invariance_guards(tmp_path):
    ...
    # Corrupted root visits vs completed
    checkpoint = read_json(tmp_path / "mcts_state.json")
    checkpoint["nodes"][0]["visits"] = 99  # Corrupt root visits
...
def test_amend_budget_shrinking_rejected(tmp_path):
    ...
    # Shrinking budget from 4 to 2 steps must fail
    with pytest.raises(ValueError, match="a budget can only grow"):
        amend_budget(tmp_path, steps=2, minutes={})

    # Growing budget from 4 to 6 steps must succeed and update run_config.json
